...
export const fetchEmail = catchAsync(async (req, res, next) => {
  ...
  if (user.length > 1 && !req.body.middleName && !req.body.birthDate)
...
export const fetchUserWithProvidedPersonalData = catchAsync(
  async (req, res, next) => {
    if (req.skipMiddleWare) return next();
    if (!req.body.password)
      return next(
        new AppError("Please Provide the password filed with other Data", 404)
      );
    const user = await User.findAll({
      where: {
        email: req.body.email,
    ...
    if (user.length > 1 && !req.body.userName && filterByMiddleName!.length > 1)
      return res.status(409).json({
        status: "error",
...
export const fetchUserWithProvidedUserName = catchAsync(
  async (req, res, next) => {
    if (req.skipMiddleWare) return next();
    if (!req.body.password)
